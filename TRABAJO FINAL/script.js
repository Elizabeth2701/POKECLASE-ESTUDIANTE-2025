...
const flyingButton = document.getElementById("flying-btn");
const flyingGrid = document.getElementById("flying-grid");
...
const renderSingle = (pokemon) => {
  ...
  pokemonCard.innerHTML = `
    ...
    <div class="meta">
      <div><span>Altura</span>${(pokemon.height / 10)} m</div>
      <div><span>Peso</span>${(pokemon.weight / 10)} kg</div>
      <div><span>Base XP</span>${pokemon.base_experience}</div>
      <div><span>Habilidades</span>${abilities}</div>
    </div>
  `;
};

// --- Render de la grilla de Pokémon de fuego ---
const renderFlyingGrid = (list) => {
  if (!list.length) {
    flyingGrid.innerHTML = '<p class="placeholder">No se pudieron cargar los Pokémon voladores.</p>';
    return;
  }

  // Renderiza cada Pokémon de fuego con su tipo principal como clase para colorear la card.
  flyingGrid.innerHTML = list
    .map(
      (pokemon) => `
        <article class="card ${typeClass(pokemon.types[0].type.name )}">
          <div class="card-header">
            <div class="pill">#${pokemon.id}</div>
            <div class="types">
              ${pokemon.types
...
// --- Controlador que carga todos los Pokémon de tipo fuego ---
const handleFlying = async () => {
  flyingButton.disabled = true;
  flyingGrid.innerHTML = '<p class="placeholder">Cargando lista...</p>';
  setStatus("Cargando todos los Pokémon de dragón volador...", "info");

  try {
    // Paso 1: obtener la lista de Pokémon de tipo fuego.
    // Endpoint: GET https://pokeapi.co/api/v2/type/flying -> devuelve nombres/URLs.
    const response = await fetch(`${API_BASE}/type/flying`);
    if (!response.ok) {
    ...
    const flyingPokemons = detailResults
      .filter((result) => result.status === "fulfilled")
      .map((result) => result.value)
      .sort((a, b) => a.id - b.id);

     renderFlyingGrid(flyingPokemons);
     setStatus(`Listo: ${flyingPokemons.length} Pokémon voladores.`, "ok");
  } catch (error) {
    flyingGrid.innerHTML = '<p class="placeholder">No se pudo consultar la PokéAPI ahora.</p>';
    setStatus("No se pudieron cargar los Pokémon voladores. Intenta nuevamente.", "error");
  } finally {
    flyingButton.disabled = false;
  }
};

searchForm?.addEventListener("submit", handleSearch);
flyingButton?.addEventListener("click", handleFlying); 